...
app.post('/canchas', async (req, res) => {
    const { nombre, tipo_superficie, capacidad_personas, precio_hora, cubierta, horario_apertura, horario_cierre } = req.body; // Extraemos los datos del cuerpo de la petición
    // Validación básica
    if (!nombre || !tipo_superficie || !capacidad_personas || !precio_hora || !cubierta == null || !horario_apertura || !horario_cierre) {
        return res.status(400).json({ error: 'Todos los campos son obligatorios' });
    }
    try {
        // Usamos "?" como placeholders para prevenir inyección SQL
        const query = 'INSERT INTO canchas_deportivas (nombre, tipo_superficie, capacidad_personas, precio_hora, cubierta, horario_apertura, horario_cierre) VALUES (?, ?, ?, ?, ?, ?, ?)';
        const [result] = await db.query(query, [nombre, tipo_superficie, capacidad_personas, precio_hora, cubierta, horario_apertura, horario_cierre]);
        res.status(201).json({
...
app.put('/canchas/:id', async (req, res) => {
    ...
    try {
        // Usamos "?" como placeholders para prevenir inyección SQL
        const query = 'UPDATE canchas_deportivas SET nombre= ?, tipo_superficie=?, capacidad_personas=?, precio_hora=?, cubierta=?, horario_apertura=?, horario_cierre=?  where id = ? ';
        console.log(query);
        const [result] = await db.query(query, [nombre, tipo_superficie, capacidad_personas, precio_hora, cubierta, horario_apertura, horario_cierre, id]);
        res.status(201).json({
            mensaje: 'Usuario guardado con éxito',
            id: result.insertId // Retornamos el ID generado en la DB
        });
